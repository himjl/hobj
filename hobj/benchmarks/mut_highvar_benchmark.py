...
from hobj.benchmarks.binary_classification.benchmark import (
    LearningCurveBenchmark,
    LearningCurveBenchmarkConfig,
    TargetSubtaskData,
)
...
class MutatorHighVarBenchmark(LearningCurveBenchmark):
    def __init__(self, cachedir: Path | None = None):
        ...
        for assignment_id, session in sessions.groupby("assignment_id", sort=False):
            ...
            assert (
                len(categories) == 2
            ), f"Expected two categories, but got {categories}"
            ...
            subtask_name_to_results[subtask_name].append(
                BinaryClassificationSubtaskResult(
                    perf_seq=np.array(perf_seq), worker_id=session.worker_id.iloc[0]
                )
            )

        # Cast as numpy
        for name in subtask_name_to_results:
            subtask_name_to_results[name] = np.array(
                subtask_name_to_results[name]
            )  # [session, trial]
