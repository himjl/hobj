...
class BinaryClassificationSubtask(pydantic.BaseModel):
    ...
    classA: List[ImageId]
    classB: List[ImageId]
    ntrials: int = pydantic.Field(
        description="The number of trials in the subtask.", gt=0
    )
    replace: bool = pydantic.Field(
        description="Whether to show stimulus images with replacement or not."
    )
    ...
    def validate_image_refs(self) -> "BinaryClassificationSubtask":
        if not self.replace and len(self.classA) + len(self.classB) < self.ntrials:
            raise ValueError(
                f"Specified replace=False, but only {len(self.classA) + len(self.classB)} images available. (len(classA)={len(self.classA)}; len(classB)={len(self.classB)}"
            )
        return self
    ...
    def simulate_session(
        ...
        for i_image in i_image_seq:
            ...
            image_ref_cur = (
                self.classA[i_image]
                if i_image < num_classA
                else self.classB[i_image - num_classA]
            )
            correct_action_cur = (
                classA_correct_action if i_image < num_classA else classB_correct_action
            )
            ...
            feedback = 1.0 if a == correct_action_cur else -1.0
