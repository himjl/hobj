...
class RepresentationalModel:
    ...
    def get_features(self, image: ImageId) -> np.ndarray:
        """
        ...
        if f.shape != (self.d,):
            raise ValueError(
                f"Expected feature vector of shape {(self.d,)}, but got {f.shape}"
            )
    ...
    def from_precomputed_features(
        cls, image_ref_to_features: Dict[ImageId, np.ndarray]
        ...
        for ref in image_ref_to_features:
            ...
            if not isinstance(f, np.ndarray):
                raise ValueError(
                    f"Expected feature vector to be a np.ndarray, but got {f} of type {type(f)}"
                )
            ...
            if not f.shape[0] == d:
                raise ValueError(
                    f"Expected feature vector to be of shape ({d},), but got {f.shape}"
                )
